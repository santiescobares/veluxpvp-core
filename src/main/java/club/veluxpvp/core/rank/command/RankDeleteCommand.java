...
public class RankDeleteCommand extends RankCommand {

	@Command(name = "rank.delete", aliases = {"rank.remove"}, permission = "core.command.rank")
	public void execute(CommandArgs cmd) {
		...
		if(args.length >= 1) {
			...
			sender.sendMessage(ChatUtil.TRANSLATE("Rank &b" + rank.getName() + " &cdeleted&f!"));
			
			for(Player p : rankManager.getPlayersWithRank(rank)) {
				rankManager.updatePermissions(p);
			}
		} else {
			sender.sendMessage(ChatUtil.TRANSLATE("&cUsage: /rank delete <rankName>"));
		}
	}
}
