...
public class Menu {
	...
	private int size;
	private boolean autoUpdate, updateOnClick, fillEmptySpaces;
	...
	public void startAutoUpdate() {
		this.updateTask = new BukkitRunnable() {
			...
			@Override
			public void run() {
				Menu openedMenu = MenuManager.getOpenedMenu(viewer);
				
				if(openedMenu == null || !openedMenu.isAutoUpdate()) {
					this.cancel();
					return;
				}
				
				for(Map.Entry<Integer, Button> buttonsEntry : openedMenu.getButtons().entrySet()) {
					openedMenu.getInventory().setItem(buttonsEntry.getKey(), new ItemStack(Material.AIR));
					openedMenu.getInventory().setItem(buttonsEntry.getKey(), buttonsEntry.getValue().build());
				}
			}
			
		}.runTaskTimerAsynchronously(Core.getInstance(), 3L, 3L);
	}
